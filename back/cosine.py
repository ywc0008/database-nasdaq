...
#종가 데이터 실수화
df["stock_closing_price"]=pd.to_numeric(df["stock_closing_price"].str.replace(',', ''), errors='coerce')
...
plt.xticks(np.arange(len(target)), pd.to_datetime(target.index.values).strftime('%Y-%m-%d'), rotation=45)
